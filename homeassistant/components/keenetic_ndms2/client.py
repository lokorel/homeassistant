...
class Client:
    ...
    def get_interfaces(self) -> List[InterfaceInfo]:
        collection = _parse_collection_lines(
            self._connection.run_command(_INTERFACES_CMD)
        )

        _LOGGER.debug("Raw interfaces info: %s", str(collection))
        assert isinstance(
            collection, list
        ), "Interfaces info response is not a collection"
    ...
    def get_interface_info(self, interface_name) -> Optional[InterfaceInfo]:
        info = _parse_dict_lines(
            self._connection.run_command(_INTERFACE_CMD % interface_name)
        )
    ...
    def get_hotspot_devices(self) -> List[Device]:
        ...
        return [
            Device(
                ...
                ip=info.get("ip"),
                interface=(
                    info.get("interface").get("name", "")
                    if info.get("interface") is not None
                    else None
                ),
    ...
    def get_associated_devices(self):
        associations = _parse_dict_lines(
            self._connection.run_command(_ASSOCIATIONS_CMD)
        )
        ...
        ap_to_bridge = {}
        for ap in aps:
            ap_info = _parse_dict_lines(
                self._connection.run_command(_INTERFACE_CMD % ap)
            )
            ...
            if mac is not None and info.get("authenticated") in ["1", "yes"]:
                ...
                devices.append(
                    Device(
                        ...
                        access=host_info.get("access") if host_info else None,
                        schedule=host_info.get("schedule") if host_info else None,
                    )
                )
        ...
        for dev in l:
            ...
            if key in res:
                ...
                res[key] = Device(
                    ...
                    access=old_dev.access or dev.access,
                    schedule=old_dev.schedule or dev.schedule,
                )
            else:
                res[key] = dev
